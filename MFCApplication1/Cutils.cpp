...
size_t Cutils::vectorProduct(const std::vector<int64_t>& vector)
{
    if (vector.empty())
        return 0;
...
std::wstring Cutils::charToWstring(const char* str)
{
...
std::vector<std::string> Cutils::loadNames(const std::string& path)
{
    // load class names
...
//矩形中心点  
std::vector<int> getCenterPoint(cv::Rect rect)
{
	std::vector<int> result = { 0,0,0,0};
	result[0] = rect.x + cvRound(rect.width / 2.0);
	result[1] = rect.y + cvRound(rect.height / 2.0);
	result[2] = rect.height;
	result[3] = rect.width;
	return result;
}

std::vector<int> Cutils::visualizeDetection(cv::Mat& image, std::vector<Detection>& detections,
                               const std::vector<std::string>& classNames)
{
    ...
    for (const Detection& detection : detections)
    {
		...
		cont++;
		if (cont == 1) {
			result = getCenterPoint(detection.box);
			cont = 0;
		}
    }
	return result;
...
void Cutils::letterbox(const cv::Mat& image, cv::Mat& outImage,
...
void Cutils::scaleCoords(const cv::Size& imageShape, cv::Rect& coords, const cv::Size& imageOriginalShape)
{
...
template <typename T>
T Cutils::clip(const T& n, const T& lower, const T& upper)
{
